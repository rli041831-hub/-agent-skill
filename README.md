...
Clone this repository, then copy the [`coordinate-project-agents`](coordinate-project-agents) directory into your Codex skills directory:
...
Codex may select the skill automatically for external-agent coordination requests. To invoke it explicitly, start your request with `$coordinate-project-agents`. For example:

```text
$coordinate-project-agents Split this feature between two external coding agents. Give me self-contained messages to send them, with non-overlapping ownership and an integration plan.
```

```text
$coordinate-project-agents Review this agent's PR report against the exact commit and tests. Tell me what is verified, what is only claimed, and what remains before merge.
```

The expected output is a practical assignment or review brief, not an automatically launched agent. Codex can inspect accessible branches and test results, but you must relay messages to agents in other tools unless a direct channel is available. The skill provides optional [charter, task, review, and handoff templates](coordinate-project-agents/references/templates.md); adapt them to the actual project rather than treating them as mandatory forms.
